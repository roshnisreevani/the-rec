...
import { fetchGroupInvitePreview, GROUP_TYPE_LABELS, joinGroupViaInvite, type GroupInvitePreview } from '@/lib/groups';
...
export default function JoinGroupScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await fetchGroupInvitePreview(code);
        if (!cancelled) setPreview(result);
      } catch (e) {
        if (!cancelled) Alert.alert('Could not load invite', e instanceof Error ? e.message : 'Unknown error.');
      } finally {
  ...
  const handleJoin = async () => {
    ...
    try {
      ...
      Alert.alert('Could not join group', e instanceof Error ? e.message : 'Unknown error.');
    } finally {
      setJoining(false);
    }
  };
  ...
  if (!preview) {
    return (
      <SafeAreaView style={styles.loading} edges={['top']}>
        ...
        <AnimatedPressable style={styles.doneButton} onPress={() => router.replace('/(tabs)/groups')}>
          <Text style={styles.doneButtonText}>Back to Groups</Text>
        </AnimatedPressable>
      </SafeAreaView>
    );
  }
  ...
  if (requested) {
    return (
      <SafeAreaView style={styles.loading} edges={['top']}>
        <Text style={styles.errorTitle}>Request sent</Text>
        <Text style={styles.errorText}>
          {preview.name}&apos;s owner will need to approve your request before you can join.
        </Text>
        <AnimatedPressable style={styles.doneButton} onPress={() => router.replace('/(tabs)/groups')}>
          <Text style={styles.doneButtonText}>Back to Groups</Text>
        </AnimatedPressable>
    ...
    <SafeAreaView style={styles.flex} edges={['top']}>
      <View style={styles.content}>
        <View style={styles.avatarRow}>
          {preview.avatarUrl ? (
            <Image source={{ uri: preview.avatarUrl }} style={styles.avatar} />
          ) : (
        ...
        <View style={styles.metaRow}>
          <View style={styles.typePill}>
            <Text style={styles.typePillText}>{GROUP_TYPE_LABELS[preview.groupType]}</Text>
          </View>
        ...
        <AnimatedPressable style={styles.joinButton} onPress={handleJoin} disabled={joining}>
          {joining ? (
            ...
            <Text style={styles.joinButtonText}>
              {preview.privacy === 'private' ? 'Request to Join' : 'Join Group'}
            </Text>
          )}
        </AnimatedPressable>

        <AnimatedPressable onPress={() => router.replace('/(tabs)/groups')} hitSlop={8}>
